 ...
 */
function slugify(name, { allowUnicode } = null) {
    let value = String(name);
    if (allowUnicode)
        value = value.normalize("KFKC");
    else
...
class SchemaValidationError extends Error {
    constructor(message) {
        super(`Session configuration JSON is invalid (d626ac4b-fbef-46bf-af96-50c1d6ac926b): ${message}`);
        this.name = "SchemaValidationError";
    }
}
class ReportingError extends Error {
    constructor(message = "Programming error: cannot report over HTTP, as no endpoint is supplied") {
        super(message);
...
class QuestionnaireApp extends d {
    constructor() {
        ...
        this.start = async (event) => {
            event.preventDefault();
            window.open(this.state.currentSubject.url, this.POPUP_WINDOW, "toolbar=no,menubar=no,left=1,top=1");
        };
    }
    async componentDidMount() {
        ...
        if (!sessionKey)
            throw new Error("1753c02d-503b-4422-8a5e-2e8464cadf17: no session URL provided");
